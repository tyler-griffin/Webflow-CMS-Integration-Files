	...
	<div class="profile-meta-info-wrapper">
		...
		<? if($itemImage) { ?>
			<div class="profile-image-wrapper">
				<div class="amsd-image staff" style="background-position: <?= $itemImage->config->{'background-position'} ?>; background-image: url('/image/<?= $itemImage->id ?>/800');"></div>
			</div>
		<? } ?>
				...
				<div class="amsd-item <?= $CLASS_GRID ?>">
					...
					<? if($itemImage) { ?>
						<a <? if($link) { ?>href="<?= $link ?>"<? } ?> class="amsd-image-link w-inline-block <?= $CLASS_GRID ?>" title="<?= $ITEM->title ?>">
							<div class="amsd-image <?= $CLASS_GRID ?> staff" style="background-position: <?= $itemImage->config->{'background-position'} ?>; background-image: url('/image/<?= $itemImage->id ?>/800');">
